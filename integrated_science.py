...
def create_contextual_model(text):
    openai.api_key = 'API를 입력하시면 됩니다'
    messages = [
        {"role": "system", "content": "You are a helpful assistant with extensive knowledge of the following text."},
        {"role": "user", "content": f"You can study every detail of this text closely to answer all your questions related to its content(say everything in korean, You do not need to reveal that you have studied this text.(example : do not say 'according to this text', '텍스트에 따르면')) : '''{text}'''"},
